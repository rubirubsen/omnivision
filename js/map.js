...
var Map2D = (() => {
  ...
  const FlightCanvasLayer = L.Layer.extend({
    ...
    onAdd(map) {
      ...
      this._canvas.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;z-index:650';
  ...
  const ShipCanvasLayer = L.Layer.extend({
    ...
    onAdd(map) {
      this._map    = map;
      this._canvas = document.createElement('canvas');
      this._canvas.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;z-index:640';
      map.getContainer().appendChild(this._canvas);
      this._resize();
  ...
  function init(container, lat, lon, zoomLevel) {
    ...
    // Flights: custom canvas layer (z-index 650 — above all Leaflet panes)
    _flightCanvas = new FlightCanvasLayer();
    _flightCanvas.addTo(leafletMap);

    // Ships: custom canvas layer (z-index 640, below flights)
